...
export abstract class PageNumControllable {
  ...
  async getCurrentPage() {
    const handler = this.page.locator(
      `.list-pagination-box>.ant-pagination-item-active`,
    );
  ...
  async gotoPage(num: number) {
    const handler = this.page.locator(
      `.list-pagination-box>.ant-pagination-item[title='${num}']`,
    );
  ...
  async hasNextPage(): Promise<boolean | never> {
    ...
    let className = await handler
      .getAttribute("class", { timeout: ACTION_TIMEOUT })
      .catch(() => {});
  ...
  async isAuth() {
    return /safe.liepin.com\/page\/liepin\/captchaPage_ip_PC/.test(
      this.page.url(),
    );
  }
  async close() {
