...
const EDU_KEYS: readonly string[] = Object.keys(Education).filter(
  (k) => !/\d/.test(k),
);
export class DataParser {
  ...
  static paseSalary(str: string) {
    ...
    if (res) {
      ...
      return {
        salaryMin: parseInt(res.min) * 22,
        salaryMax: parseInt(res.max) * 22,
        salaryMonth: 12,
      };
    }
  ...
  static isCity(str: string) {
    for (const { name } of cities) {
      ...
      if (name.includes(str)) {
        ...
        return (name.length / str.length) * 2 - 0.5;
    }
    return 0;
  }
}
