...
describe("parseProcessArgs", function () {
  ...
  it("无key", function () {
    expect(parseProcessArgs(["sg", "dg"])).toEqual({
      default: ["sg", "dg"],
      map: {},
    });
  });
  describe("boolean参数", function () {
    it("纯布尔", function () {
      expect(parseProcessArgs(["-a", "-b"])).toEqual({
        default: [],
        map: { a: true, b: true },
      });
    });
    it("连续的", function () {
      expect(parseProcessArgs(["-abc"])).toEqual({
        default: [],
        map: { a: true, b: true, c: true },
      });
    });
    it("带参数", function () {
      expect(parseProcessArgs(["-a", "1"])).toEqual({
        default: [],
        map: { a: "1" },
      });
    });
  ...
  describe("键值对", function () {
    it("纯布尔", function () {
      expect(parseProcessArgs(["--aa", "--bb"])).toEqual({
        default: [],
        map: { aa: true, bb: true },
      });
    });
    ...
    it("带参数", function () {
      expect(parseProcessArgs(["--aa", "1"])).toEqual({
        default: [],
        map: { aa: "1" },
      });
    });
