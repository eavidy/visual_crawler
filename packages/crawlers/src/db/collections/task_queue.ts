...
export type UnexecutedTask<T extends CrawlerPriorityTask> = WithId<
  Omit<T, "priority" | "expirationTime" | "status">
>;
...
export class TaskQueueData {
  ...
  async takeTasks(count: number, siteTag: SiteTag, taskType?: TaskType) {
    ...
    let res = await this.priorityQueueView
      .find<UnexecutedCrawlerTask>(match)
      .limit(count)
      .toArray();
  ...
  async appendTask(task: CrawlerTaskAppend) {
    ...
    return taskQueueCollection.insertOne({
      ...task,
  ...
  async appendTasks(tasks: CrawlerTaskAppend[]) {
    {
      if (!tasks.length)
        throw new FieldCheckError({ length: "至少为1, 实际0" });
      let res = checkType(
        tasks,
        typeChecker.arrayType(taskChecker),
        CheckTypeOption,
      ).error;
      if (res) throw new FieldCheckError(res);
    }
  ...
  async markTasksSucceed(id: string | number | ObjectId, result?: any) {
    return taskQueueCollection.updateOne(
      { _id: toId(id) },
      { $set: { status: TaskState.executed, result } },
    );
  }
