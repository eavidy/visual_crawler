...
async function start(taskQueue: TaskQueue) {
  ...
  crawler.on("scheduleUpdate", function (this: CrawlerLiepin) {
    ...
    let yy = [
      ...
      "职位重复率:" +
        Math.round(
          (total.jobTotal ? statistics.jobRepeated / total.jobTotal : 0) * 100,
        ) +
        "%",
      "公司重复率:" +
        Math.round(
          (total.companyTotal
            ? statistics.companyRepeated / total.companyTotal
            : 0) * 100,
        ) +
        "%",
      "任务失败率:" +
        Math.round(
          (total.taskTotal ? statistics.taskFailed / total.taskTotal : 0) * 100,
        ) +
        "%\n",
      `任务进度: ${this.currentSchedule}/${this.totalSchedule} (${
        this.totalSchedule
          ? Math.round(
              (this.currentSchedule / this.totalSchedule) * 100 * 100,
            ) / 100
          : 0
      }%)\n`,
    ];
...
start(
  useNew
    ? new NewCityTaskQueue(SiteTag.liepin)
    : new TaskQueue(SiteTag.liepin, undefined, 1),
);
