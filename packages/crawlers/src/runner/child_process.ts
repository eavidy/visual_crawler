...
class AppCenter extends EventEmitter {
  ...
  async closeDevice() {
    let device =
      this.device instanceof Promise ? await this.device : this.device;
    if (!device) return;
  ...
  private async createCrawler(id: number, config: CreateCrawlerOptions) {
    ...
    crawler.on("scheduleUpdate", function (this: CrawlerLiepin) {
      AppCenter.sendParent(
        "scheduleUpdate",
        { total: this.totalSchedule, current: this.currentSchedule },
        id,
      );
    });
    ...
    crawler.on("taskFinished", function (this: CrawlerLiepin, taskResult) {
      ...
      if (taskCountLimit) {
        ...
        if (
          statistics.taskCompleted + statistics.taskFailed >=
          taskCountLimit
        ) {
          this.stopWork(true);
        }
