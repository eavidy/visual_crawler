...
import {
  ApiRes,
  CreateCrawlProcessOptions,
} from "common/request/crawler/crawl_process.js";
...
export class CrawlProcessService {
  ...
  startChildProcess(id: number) {
    ...
    if (crawlPcs.status !== CrawlerProcessStatus.stop)
      throw new BadRequestException("当前状态不能启动运行");
