...
import type { ApiReq } from "common/request/dashboard.d.ts";
import { WorkExp } from "common/request/enum.ts";
import { Body } from "./components/dash-left.tsx";
import { Education } from "common/model/index.js";
import LogoIcon from "@/components/img/logo.tsx";
...
function Header(props: { onAnalysis: (value: ApiReq.MatchFilter) => void }) {
  ...
  const { data, loading } = useRequest(async function () {
    const { data } = await $http.get<{ value: string; label: string }[]>(
      "api/dashboard/dq_options"
    );
    return data;
  });
  ...
  const { data: JobNameList } = useRequest(
    async function () {
      ...
      const {
        data: { items },
      } = await $http.get<{ items: string[] }>("api/job/search_job_name", {
  ...
  const [form] = Form.useForm<
    Omit<ApiReq.MatchFilter, "startTime" | "endTime"> & { timeRange?: number }
  >();
  ...
  function onAnalysis() {
    ...
    let onChangeVal: ApiReq.MatchFilter = timeRange
      ? Object.assign(values, timeRange)
      : values;
  ...
  function formatTimeRange(e?: Dayjs[]) {
    if (!e) return;
    let startTime = e[0]
      ? new Date(e[0].get("year"), 0, 1, 0, 0, 0, 0).getTime()
      : undefined;
    let endTime = e[1]
      ? new Date(e[1].get("year"), 11, 31, 23, 59, 59, 999).getTime()
      : undefined;
      ...
      >
        ...
        <div style={{ display: "flex", gap: "8px", paddingRight: 20 }}>
          ...
          <Form.Item
            name="timeRange"
            getValueProps={(e) => ({})}
            getValueFromEvent={formatTimeRange}
          >
