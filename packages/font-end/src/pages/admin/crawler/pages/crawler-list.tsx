...
import type { ApiReq } from "common/request/crawler/crawler.d.ts";
import { useRequest } from "ahooks";
import { crawlerResource, CrawlerInfo } from "../services/crawler.resource.ts";
import { useNavigate, useParams } from "react-router-dom";
import { CrawlerProcessStatus, CrawlerStatus } from "common/request/enum.ts";
import { CrawlerCard } from "../components/crawler-card.tsx";
...
export default function CrawlerList() {
  ...
  return (
    <PageContainer backIcon={<LeftOutlined />} title={processName} onBack={() => navigate(-1)}>
      ...
      >
        <CrawlerCardList data={group[currentTab]} pcId={processId!} pcStatus={processStatus} onRefresh={refresh} />
      </Card>
      {processId !== undefined && (
        <CrawlerAddModal
...
function CrawlerAddModal(props: ProcessAddModalProps) {
  ...
  const typeDesc = "创建";
  const { loading, runAsync } = useRequest((data: ApiReq.CreateCrawler) => crawlerResource.createCrawler(pcId, data), {
    manual: true,
    onSuccess() {
    ...
    () =>
      data.map((item) => (
        <CrawlerCard
          ...
          {...item}
          onDelete={onDelete}
          onStart={(id: any) => onUpdateStart(id, true)}
          onStop={onStop}
        />
      )),
    [data]
