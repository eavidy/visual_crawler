...
import type { ApiRes } from "common/request/crawler/crawl_process.d.ts";
import { DefaultStatus, FinishedStatus } from "@/components/status-bar.tsx";
...
import { CrawlerProcessStatus } from "common/request/enum.ts";
          ...
          <Space>
            ...
            {startOrStop && (
              <Tooltip title={startOrStop}>
                {startOrStop === "停止" ? (
                  <StopOutlined style={{ cursor: "pointer" }} onClick={() => onStopOrStart(record, false)} />
                ) : (
                  <PlaySquareOutlined style={{ cursor: "pointer" }} onClick={() => onStopOrStart(record, true)} />
                )}
              </Tooltip>
            )}
