...
export type TaskQueueList = Required<ApiRes.GetTaskQueueList> & { taskInfo: string; key: string };
...
class TaskQueueResource {
    async getTasks(
        ...
        const { data } = await $http.get<{ items: ApiRes.GetTaskQueueList[]; total: number }>("/api/task_queue/list", {
